...
<<echo=F, results=hide>>=
set.seed(54321)
turnout <- rbinom(500, size = 1, prob = .55)
options(width = 80)
@ 
...
\medskip
\tiny
<<fig = T, include = F, label = trianglepost>>=
plot(x = grid.points, y = post.ord, type = "l", col = "blue", xlab =
  expression(pi), ylab = expression(paste("p(", pi, "|y)")), main =
...
\tiny
<<>>=
grid.a <- seq(from = mle[1] - 5*mle.se[1], to = mle[1] + 5*mle.se[1],
length.out = 200)
grid.b <- seq(from = mle[2] - 5*mle.se[2], to = mle[2] + 5*mle.se[2],
length.out = 200)
...
\medskip
<<>>=
post.ord <- apply(grid.points, MARGIN = 1, FUN = poisson.posterior, y
                  = y.vec, x = x.vec, prior.mean.a = mu.a, prior.var.a =
                  sigma2.a, prior.mean.b = mu.b, prior.var.b = sigma2.b)
@ 
\pause
\bigskip
\normalsize
...
\tiny
<<>>=
posterior.means <- colMeans(sim.posterior); posterior.means
mle
@
\normalsize
\bigskip
...
\tiny
<<>>=
posterior.sd <- apply(sim.posterior, MARGIN = 2, FUN = sd); posterior.sd
mle.se
@ 
\end{frame}
