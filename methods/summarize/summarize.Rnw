...
\tiny
<<>>=
quantile(posterior.unif.prior, probs = c(.025,.975))
@ 
\normalsize
\end{frame}
...
\begin{frame}[fragile]
\tiny
<<results = hide, fig = T, include = F, label = hpd1>>=
hdr.den(posterior.unif.prior, prob = 95, main = "95% HPD region", xlab = expression(pi), ylab = expression(paste("p(", pi, "|y)")))
@ 
\begin{figure}[!htp]
\begin{center}
...
<<results = hide, fig = T, echo = F, include = F, label = hpd2>>=
xx <- c(rbeta(1000,3,8), rbeta(1000,9,1))
hdr.den(xx, prob = 95, main = "95% HPD region", xlab = expression(pi), ylab = expression(paste("p(", pi, "|y)")))
@ 
\begin{figure}[!htp]
\begin{center}
