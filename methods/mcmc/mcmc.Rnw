...
@ 
<<>>=
gibbs <- function(n.sims, beta.start, alpha, gamma, delta, y, t,
burnin = 0, thin = 1){
	...
	for(i in 1:n.sims){
          ...
          beta.cur <- beta.update(alpha = alpha, gamma = gamma, delta
= delta, lambda = lambda.cur, y = y)
...
\tiny
<<>>=
posterior <- gibbs(n.sims = 10000, beta.start = 1, alpha = 1.8, gamma
= 0.01, delta = 1, y = y, t = t)
...
theta.update <- function(theta.cur, shape, rate){
  ...
  accept.prob <- dgamma(theta.can, shape=shape,
                      rate=rate)/dgamma(theta.cur, shape=shape,
                        rate=rate)
