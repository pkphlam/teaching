...
<<>>=
library(MCMCpack)
#posterior <- MCMClogit(new.onset ~ warl + gdpenl + lpopl1 +
#lmtnest + ncontig + Oil + nwstate + instab + polity2l + ethfrac +
#                       relfrac, data=data, tune = .6, burnin = 1000, mcmc=5000)
@ 
\normalsize
\end{frame}
...
\pause
<<>>=
#X <- cbind(1,data[,c("warl", "gdpenl","lpopl1",
...
#emp.year.sum[i] <- sum(data$new.onset[which(data$year==unique(data$year)[i])])
#}
...
#for(i in 1:length(unique(data$year))){
#year.sum[i,] <- apply(y.rep[which(data$year==unique(data$year)[i]),], 2, sum)
#}
...
@
\end{enumerate}
<<echo = F, fig = T, include = F, label = yearhist>>=
#hist(T.y.rep, xlab = "test statistic", main = "")
#abline(v = T.y, col = "red")
#legend(x="topright", legend="Observed Test Statistic", col = "red",
#       lty=1, bty = "n")
@ 
\begin{figure}[!htp]
