...
\begin{frame}
<<echo=F, label=prior1, include=F, fig=T>>=
x <- seq(0,1,.01)
prior1 <- dbeta(x, 1,1)
like <- 65/82
post1 <- dbeta(x, 65+1, 17+1)
plot(x=x, y=post1, type="l", xlab=expression(pi), ylab="", col="blue",
     main="Uninformative Beta(1,1) Prior")
lines(x=x, y=prior1, col="red")
abline(v=like, lty=2)
legend(x="topleft", legend=c("Prior", "Data (MLE)", "Posterior"),
       lty=c(1,2,1), col=c("red","black","blue"))
@ 

<<echo=F, label=prior2, include=F, fig=T>>=
x <- seq(0,1,.01)
prior2 <- dbeta(x, 2,12)
like <- 65/82
post2 <- dbeta(x, 65+2, 17+12)
plot(x=x, y=post2, type="l", xlab=expression(pi), ylab="", col="blue",
     main="Beta(2,12) Prior")
lines(x=x, y=prior2, col="red")
abline(v=like, lty=2)
abline(v=67/(67+29), col="blue")
legend(x="topleft", legend=c("Prior", "Data (MLE)", "Posterior"),
       lty=c(1,2,1), col=c("red","black","blue"))
@ 

<<echo=F, label=prior3, include=F, fig=T>>=
x <- seq(0,1,.01)
prior3 <- dbeta(x, 1,1)
like <- 790/1000
post3 <- dbeta(x, 791, 211)
plot(x=x, y=post3, type="l", xlab=expression(pi), ylab="", col="blue",
     main="Uninformative Beta(1,1) Prior (n=1000)")
lines(x=x, y=prior3, col="red")
abline(v=like, lty=2)
legend(x="topleft", legend=c("Prior", "Data (MLE)", "Posterior"),
       lty=c(1,2,1), col=c("red","black","blue"))
@ 

<<echo=F, label=prior4, include=F, fig=T>>=
x <- seq(0,1,.01)
prior4 <- dbeta(x, 2,12)
like <- 790/1000
post4 <- dbeta(x, 792, 222)
plot(x=x, y=post4, type="l", xlab=expression(pi), ylab="", col="blue",
     main="Beta(2,12) Prior (n=1000)")
lines(x=x, y=prior4, col="red")
abline(v=like, lty=2)
abline(v=792/(792+222), col="blue")
legend(x="topleft", legend=c("Prior", "Data (MLE)", "Posterior"),
       lty=c(1,2,1), col=c("red","black","blue"))
@ 
